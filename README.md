...
│   │   ├── __init__.py
│   │   ├── settings.py             # All settings (DB, JWT, M-Pesa, CORS, email)
...
│   │   ├── models.py               # All data models (see Models section)
│   │   ├── serializers.py          # DRF serializers
│   │   ├── views.py                # ViewSets and API views
│   │   ├── urls.py                 # App-level URL patterns
│   │   └── filters.py              # (optional) Custom django-filter classes
│   │
...
│   ├── .gitignore
│   ├── db.sqlite3                  # SQLite dev database (gitignored in prod)
│   ├── manage.py
│   └── requirements.txt
│
└── README.md                       # This file
...
# ── M-Pesa Daraja API ─────────────────────────────────────────────────────────
# Credentials from https://developer.safaricom.co.ke → Your App → Keys
MPESA_CONSUMER_KEY=your_consumer_key
MPESA_CONSUMER_SECRET=your_consumer_secret

# Sandbox shortcode — use your actual PayBill/Till for production
...
| `Review` | User reviews with rating (1–5), verified purchase flag |
...
| Method | Endpoint | Description |
|---|---|---|
| `POST` | `/mpesa/stk-push/` | Initiate STK Push to customer's phone (🔒) |
...
> ⚠️ The ngrok URL changes each session unless you have a paid plan. Update `.env` and restart Django each time.
...
**Cause:** `.env` values not loaded — `MPESA_CONSUMER_KEY` is empty string.
...
- Network blocked — test with `curl`:
...
- ngrok URL has changed — update `.env` and restart Django
- Callback URL path mismatch — must be exactly `/api/v1/mpesa/callback/`
